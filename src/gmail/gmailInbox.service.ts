        ...
        try {
            ...
            for (let index = 0; index < data?.messages?.length; index++) {
                ...
                if (element.payload?.parts) {
                    ...
                    const bodyData = parts[0].body.data;
                    const decodedResponse = Buffer.from(bodyData, 'base64').toString('utf-8');
                    message.push(decodedResponse);
                    console.log('message::', message);

                } else {
                    // If payload doesn't have parts, use payload body data
                    const bodyData = element.payload?.body.data;
                    if (bodyData) {
                        const decodedResponse = Buffer.from(bodyData, 'base64').toString('utf-8');
                        message.push(decodedResponse);
                        console.log('message::', message);
